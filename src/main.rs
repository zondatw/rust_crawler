...
fn get_last_page_num(document: &Document) -> i32 {
    document
        .find(And(Name("span"), Class("last-page")))
        .map(|node| node.text())
        .collect::<String>()
        .trim()
        .parse::<i32>()
        .expect("Get last page number error")
}
...
fn get_vul_info(vul_report_url: &str) -> HashMap<String, String> {
    ...
    vul_info.insert(
        String::from("title"),
        report_document
            .find(And(Name("li"), Class("title")).descendant(And(Name("span"), Class("value"))))
            .map(|node| node.text())
            .collect::<String>()
            .trim()
            .to_string(),
    ...
    vul_info.insert(
        ...
        report_document
            .find(And(Name("div"), Class("status-descr")))
            .map(|node| node.text())
            .collect::<String>()
            .split(" : ")
            .collect::<Vec<&str>>()[1]
    ...
    vul_info.insert(
        String::from("status"),
        report_document
            .find(And(Name("div"), Class("status-label")))
            .map(|node| node.text())
            .collect::<String>(),
    );
...
fn parse_block(document: &Document) -> bool {
    ...
    for node in document.find(And(Name("li"), Attr("class", "strip"))) {
        ...
        let naivedate_last_update = NaiveDate::parse_from_str(
            vul_info.get("last_update").expect("last_update not exist"),
            "%Y/%m/%d",
        )
        ...
        if last_update >= yesterday {
            ...
            println!(
                "\tTitle: {}",
                vul_info.get("title").expect("title not exist")
            );
            println!(
                "\tLast update: {}",
                vul_info.get("last_update").expect("last_update not exist")
            ...
            println!(
                "\tStatus: {}",
                vul_info.get("status").expect("status not exist")
            );
...
fn parse_content(body: String) {
    ...
    for page in 1..last_page_num {
        let base_url =
            Url::parse("https://zeroday.hitcon.org/vulnerability/all/").expect("Parse url error");
        let page_url = base_url
            .join(&format!("page/{}", page))
            .expect("Join url error");
